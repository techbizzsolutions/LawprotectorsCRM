...
@IonicPage()
@Component({
  selector: 'page-listcallinfo',
  templateUrl: 'listcallinfo.html',
})
export class ListcallinfoPage {
  lists=[];
  From:any = 'From';
  To:any = 'To';
  ...
  getBranch()
  {
    ...
    let url = (user.role =='3')?'getspotinfo':'getcallinfo';
    this.api.post(url,{
      // 'fromdate': this.From,
      // 'todate': this.To
    }).subscribe(res => {
  ...
  {
    item.action= 0;
    this.navCtrl.push('SpotVisitInfoTcPage',item);
  }
  ...
  {
    this.loader.Show("Loading...");
    this.api.postWithoutEmp('deletecallinfo', {
      "cid": id
    }).subscribe(res => {
      console.log('deletecallinfo', res);
      this.loader.Hide();
      if (res.status) {
        let toast = this.toastCtrl.create({
          message: res.message,
